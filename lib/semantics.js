...
const Semantics = {
  ...
  uninary: (operator) => (operand) => {
    return Maybe.flatMap(Semantics.evaluate(operand))(arg => {
  ...
  evaluate: (anExp) => (env) => {
    ...
    return Exp.match(anExp,{
      ...
      // 配列型の評価
      array: (value) => { return Maybe.just(value); },
      // オブジェクト型の評価
      ...
      let: (variable, declaration, body) => {
        return Semantics.evaluate(Exp.app(Exp.lambda(variable, body), declaration))(env);
      },
      // succ:: Exp -> Reader[Maybe[Value]]
      succ: (operand) => {
        return Maybe.flatMap(Semantics.evaluate(operand)(env))(arg => {
          return Maybe.just(1 + arg); 
        });
      },
      prev: (exp) => {
        return Maybe.flatMap(Semantics.evaluate(exp)(env))(arg => {
      ...
      binOperator: (symbol) => {
        switch(symbol) {
          ...
          default:
              return Maybe.nothing(`未知の演算子 ${symbol} です`);
        }
      },
      binArithmetic: (binOperator, expL, expR) => {
        return Maybe.flatMap(Semantics.evaluate(binOperator)(env))(operator => {
          return Maybe.flatMap(Semantics.evaluate(expL)(env))(valueL => {
            return Maybe.flatMap(Semantics.evaluate(expR)(env))(valueR => {
              return operator(valueL, valueR); // Reader[Maybe[VALUE]]
            });
          });
        });
      },
