...
const Semantics = {
  ...
  evaluate: (anExp) => {
    ...
    return Exp.match(anExp,{
      ...
      /* 関数定義（λ式）の評価  */
      // lambda:: (Var, Exp) -> Reader[Maybe[FUN[VALUE -> VALUE]]]
        ...
        return Exp.match(identifier,{
          variable: (name) => {
            const closure = ((actualArg) => {
              ...
              //   return Maybe.flatMap(Semantics.evaluate(body).run(localEnv))(value => {
              //     return Reader.unit(Maybe.just(value));
              //     // return value;
              //   });
      ...
      binOperator: (symbol) => {
        switch(symbol) {
          case "+":
            return Reader.unit(Maybe.just((x,y) => {
              return Reader.unit(Maybe.just(x + y));
              // return x + y;
            }));
            // break;
          ...
          default:
              return Reader.unit(Maybe.nothing(`未知の演算子 ${symbol} です`));
        }
      },
      binArithmetic: (binOperator, expL, expR) => {
        return Reader.flatMap(Semantics.evaluate(binOperator))(maybeOperator => {
          return Reader.flatMap(Semantics.evaluate(expL))(maybeL => {
            return Reader.flatMap(Semantics.evaluate(expR))(maybeR => {
              return Maybe.flatMap(maybeOperator)(operator => {
                return Maybe.flatMap(maybeL)(x => {
                  return Maybe.flatMap(maybeR)(y => {
                    return operator(x,y); // Reader[Maybe[VALUE]]
                  });
                });
                // return Reader.unit(Maybe.just(answer));
              });
            });
          });
        });
