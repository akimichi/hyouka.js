    ...
    return Parser.flatMap(operator())(operator => {
      return Parser.flatMap(Parser.bracket(open, operands, close))(args => {
        return Exp.match(operator, {
          variable: (name) => { // e.g.  add(1 2)
            const x = Exp.variable('x');
            const application = array.foldr(args)(Exp.lambda(x, Exp.app(operator, x)))(arg => {
              return (accumulator) => {
          ...
          lambda: (variable, body) => {
            const x = Exp.variable('x');
            const lambdaTree = array.foldr(args)(Exp.lambda(variable, body))(_ => {
              return (accumulator) => {
                return Exp.lambda(x, accumulator)
              };
            });
            const application = array.foldr(args)(lambdaTree)(arg => {
              return (accumulator) => {
                Exp.app(
                  accumulator,
                  arg
        ...
        // const application = array.foldr(args)(operator)(arg => {
        //   return (accumulator) => {
        //     return accumulator(operator, arg);
        //   };
        // });
        // return Parser.unit(application);
      });
